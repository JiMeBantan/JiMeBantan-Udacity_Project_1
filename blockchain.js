...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        if (self.height > 0) {
            block.previousBlockHash = self.chain[self.getChainHeight()].hash;
        }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let findBlock = self.chain.filter((block) => {
    ...
    /*getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
            let data = self.chain.filter((block)=> {
                block.getData();
